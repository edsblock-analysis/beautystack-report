...
const REPORTS_DIR = path.join(__dirname, 'live-site-reports');
const OUT_FILE = path.join(__dirname, 'live-site-index.html');

// Friendly site labels keyed by the slug derived from each live URL hostname.
const SITE_LABELS = {
  cutex: 'Cutex',
...
function siteFromUrl(url) {
  ...
  if (host.includes('lottabody')) return 'lotta_body';
  if (host.includes('cutex')) return 'cutex';
  if (host.includes('rouxbeauty')) return 'rouxbeauty';
  if (host.includes('sinfulcolors')) return 'sinfulcolors';
  return 'other';
}
...
function build() {
  if (!fs.existsSync(REPORTS_DIR)) {
    console.warn('No live-site-reports/ folder found.');
    return;
  }
  ...
  console.log(`Dashboard written to live-site-index.html`);
...
<title>Lighthouse Dashboard — Live Sites</title>
<style>
:root{
...
<body>
<header>
  <h1>Lighthouse Performance Dashboard — Live Sites</h1>
  <p>Mobile audits across live production sites · <span id="meta"></span></p>
</header>
...
function renderTable(){
  ...
  tb.innerHTML=rows.map(p=>{
    ...
    const href='live-site-reports/'+encodeURIComponent(p.file);
    return '<tr>'+
